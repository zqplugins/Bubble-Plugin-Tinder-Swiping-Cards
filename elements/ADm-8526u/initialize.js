function (instance, context) {
	...
	instance.data.stateHandler = function (el, type) {
		...
		if (type === "like") {
			...
			instance.publishState(
				"disliked_cards",
				Array.from(instance.data.dislikeCards)
			);
	...
	instance.data.initStateCards = function (arrList) {
		// console.log("initState Cards", arrList);
	...
	function getUpperCard(cards) {
		...
		cards = cards ?
			cards :
			instance.data.tinderContainer.querySelectorAll(
				".tinder--card" + instanceID + ":not(.removed)"
			);
		const upperCard = [...cards].find(
			(el) => Number(el.dataset.index) === instance.data.indexUpperCard
		);
		return upperCard;
	}
	...
	function getCardsInTheOrder() {
		const instanceID = instance.data.id;
		const leftCardsElementsParent = instance.data.tinderContainer.querySelector(
			".tinder--card" + instanceID + ":not(.removed)"
		)?.parentElement;
		if (!leftCardsElementsParent) return [];
		return Array.from(instance.data.leftCards).reduce((acc, index) => {
			const el = leftCardsElementsParent.querySelector(
				'[data-index="' + index + '"]'
			);
			if (el) acc.push(el);
			return acc;
	...
	instance.data.createButtonListener = function (love) {
		return function (event) {
			const instanceID = instance.data.id;
			var cards = instance.data.tinderContainer.querySelectorAll(
				".tinder--card" + instanceID + ":not(.removed)"
			);
	...
	instance.data.sendFrontToBack = function () {
		return function (event) {
			const instanceID = instance.data.id;
			const cards = instance.data.tinderContainer.querySelectorAll(
				"div.tinder--card" + instanceID + ":not(.removed)"
			);
	...
	instance.data.reloadCards = function () {
		...
		return function (event) {
			...
			var cards = instance.data.tinderContainer.querySelectorAll(
				"div.tinder--card" + instanceID + ".removed"
			);
			cards.forEach((card) => {
	...
	instance.data.hammertime_panned_handler = function (el) {
		return function (event) {
			...
			if (keep) {
				...
				if (toX > 0 && toX < clientWidth + elementWidth)
					toX += clientWidth + elementWidth;
				else if (toX < 0 && toX > -elementWidth + toX)
					toX -= clientWidth + elementWidth;

				if (toY > clientHeight)
					toY = clientHeight - elementWidth - elementWidth / 50;
